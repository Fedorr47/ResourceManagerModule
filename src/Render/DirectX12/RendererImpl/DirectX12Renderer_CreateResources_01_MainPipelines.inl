...
//Debug cubemap atlas pipeline(fullscreen triangle with a tiny VB).
{
		...
		// Deferred rendering (DX12): GBuffer writer + fullscreen resolve.
		{
			...
			const auto fxaaPath = corefs::ResolveAsset("shaders\\FXAA_dx12.hlsl");
			...
			{
				const auto vsFXAA = shaderLibrary_.GetOrCreateShader(ShaderKey{
					.stage = rhi::ShaderStage::Vertex,
					.name = "VS_Fullscreen",
					.filePath = fxaaPath.string(),
					.defines = {},
					.shaderModel = rhi::ShaderModel::SM6_1
					});
				const auto psFXAA = shaderLibrary_.GetOrCreateShader(ShaderKey{
					.stage = rhi::ShaderStage::Pixel,
					.name = "PS_FXAA",
					.filePath = fxaaPath.string(),
					.defines = {},
				...
				psoFXAA_ = psoCache_.GetOrCreate("PSO_FXAA", vsFXAA, psFXAA);
			}
